	...
	shape.prototype.init_buffers = function()			// Send the completed vertex and index lists to their own buffers in the graphics card.
		{
			...
			gl.bufferData( gl.ARRAY_BUFFER, flatten(this.texture_coords), gl.DYNAMIC_DRAW );
...
windmill.prototype.populate = function( recipient, points_transform)
{
	...
	for (var i = 0,i_max=4;i<i_max;i++)
	{
		...
		recipient.vertices.push(vec3(newPoint[1],1,newPoint[2]));
...
//from this is the new shape
//=================================================================================================================================================================

function sea(points_transform )
{
	shape.call(this);
	if(!arguments.length) return;
	this.populate(this, points_transform);
...
sea.prototype.populate = function(recipient, points_transform)
{
	var verX = [];
	var verY = [];
	var verZ = [];
	...
	{
		recipient.vertices.push(vec3(verX[k], verY[k],verZ[k]));
	};
	...
	for (var n = 0; n <= 99; n++)
	{
		for (var f = 0; f <= 99; f++)
		{
			recipient.texture_coords.push(vec2(0.01*f, 0.005*n));
			recipient.normals.push(vec3(0,1,0));
		};
	};
	...
	for (var l = 0; l <= 98; l++)
	{
		for (var m = 0; m <= 98; m++)
		{
			if( (l+m)%2 == 0 )
			{
				recipient.indices.push(100*l+m);
				recipient.indices.push(100*l+m+1);
				recipient.indices.push(100*(l+1)+m);
				recipient.indices.push(100*l+m+1);
				recipient.indices.push(100*(l+1)+m);
				recipient.indices.push(100*(l+1)+m+1);
	...
	// recipient.flat_normals_from_triples(0);
}

sea.prototype.draw = function( model_transform, camera_transform, projection_transform, texture_filename ,time)
{
	this.update_uniforms(model_transform, camera_transform, projection_transform);
	...
	for(var i = 0; i <= 99; i++)
	{
		for(var j = 0; j <= 99; j++)
		{
			...
			else if (i == 1 || i ==98 || j == 1 || j == 98)
			{
				verX[100*i+j] = j;
				verY[100*i+j] = (0.5*Math.sin(time+j/4)-0.4*Math.cos(time-i/6))/2;
				verZ[100*i+j] = i;
			}
			else
			{
				verX[100*i+j] = j;
				verY[100*i+j] = 0.5*Math.sin(time+j/4)-0.4*Math.cos(time-i/6);
				verZ[100*i+j] = i;	
			};
	    };
	};
	...
	for (var k = 0; k <= 9999; k++)
	{
		verticesUp.push(vec3(verX[k], verY[k],verZ[k]));
	};

	gl.bindBuffer( gl.ARRAY_BUFFER, this.position_buffer      );
	gl.bufferData( gl.ARRAY_BUFFER, flatten(verticesUp), gl.DYNAMIC_DRAW );
	gl.vertexAttribPointer( g_addrs.vPosition, 3, gl.FLOAT, false, 0, 0 );
...
function butterflyWing(points_transform)
{
	shape.call(this);
	if(!arguments.length) return;
	this.populate(this,points_transform);
...
inherit(butterflyWing, shape);

butterflyWing.prototype.populate = function( recipient, points_transform)
{
	var butterFlyWing = [
		0,0,0, //0
		0.5,0,-0.7, //1
	...
	var wingNormal = vec3(0,1,0);

	for(var i=0, i_max = 11; i <= i_max; i++)
	{
		recipient.vertices.push(vec3(butterFlyWing[3*i],butterFlyWing[3*i+1],butterFlyWing[3*i+2]));
		for (var j = 0; j <= 2; j++) {
			recipient.normals.push(wingNormal);
		};
		recipient.texture_coords.push(vec2(0,0));
	...
	recipient.indices.push(1);
	recipient.indices.push(4);
	recipient.indices.push(5);

	recipient.indices.push(6);
	recipient.indices.push(2);
	recipient.indices.push(3);

	recipient.indices.push(4);
	recipient.indices.push(7);
	recipient.indices.push(3);
	...
	recipient.indices.push(4);
	recipient.indices.push(5);
	recipient.indices.push(9);

	recipient.indices.push(4);
	recipient.indices.push(8);
	recipient.indices.push(9);
	...
	recipient.indices.push(6);
	recipient.indices.push(7);
	recipient.indices.push(10);

	recipient.indices.push(8);
	recipient.indices.push(9);
	recipient.indices.push(11);
}

//===================================================================================================================================================================

function Flower(points_transform)
{
	shape.call(this);
	if(!arguments.length) return;
	this.populate(this,points_transform);
...
inherit(Flower,shape);

Flower.prototype.populate = function(recipient, points_transform)
{
	var vertices = 
	[
		0,-0.5,0,//0
	...
	for(var i = 0, i_max = 35; i <= i_max; i++)
	{
		recipient.vertices.push(vec3(vertices[3*i],vertices[3*i+1],vertices[3*i+2]));
		recipient.texture_coords.push(vec2(0,0));
		recipient.texture_coords.push(vec2(0,1));
		recipient.texture_coords.push(vec2(1,0));
	};	
	for(var i = 0, i_max = 35; i <= i_max; i++)
	{
		recipient.vertices.push(vec3(-vertices[3*i+2],vertices[3*i+1],vertices[3*i]));
		recipient.texture_coords.push(vec2(0,0));
		recipient.texture_coords.push(vec2(0,1));
		recipient.texture_coords.push(vec2(1,0));
	};
...
Flower.prototype.draw = function( model_transform, camera_transform, projection_transform, texture_filename ,time)
{
	this.update_uniforms(model_transform, camera_transform, projection_transform);
	...
	[
		0,-0.5,0,//0
		-0.45,-0.2+0.1*Math.sin(time),-0.5,//1
		0.45,-0.2+0.1*Math.sin(time),-0.5,//2

		-0.45,-0.2+0.1*Math.sin(time),-0.5,//1
		0.45,-0.2+0.1*Math.sin(time),-0.5,//2
		0,0.2*Math.sin(time)-0.2,-1.1,//3

		-0.45,-0.2+0.1*Math.sin(time),-0.5,//1
		0,0.2*Math.sin(time)-0.2,-1.1,//3
		-0.85,0.25*Math.sin(time)-0.2,-1,//4

		0.45,-0.2+0.1*Math.sin(time),-0.5,//2
		0,0.2*Math.sin(time)-0.2,-1.1,//3
		0.85,0.25*Math.sin(time)-0.2,-1,//5

		0,0.2*Math.sin(time)-0.2,-1.1,//3
		-0.85,0.25*Math.sin(time)-0.2,-1,//4
		-0.8,0.3*Math.sin(time)-0.2,-1.25,//6

		0,0.2*Math.sin(time)-0.2,-1.1,//3
		0.85,0.25*Math.sin(time)-0.2,-1,//5
		0.8,0.3*Math.sin(time)-0.2,-1.25,//7

		0,0.2*Math.sin(time)-0.2,-1.1,//3
		-0.8,0.3*Math.sin(time)-0.2,-1.25,//6
		-0.7,0.3*Math.sin(time)-0.2,-1.5,//8

		0,0.2*Math.sin(time)-0.2,-1.1,//3
		0.8,0.3*Math.sin(time)-0.2,-1.25,//7
		0.7,0.3*Math.sin(time)-0.2,-1.5,//9

		0,0.2*Math.sin(time)-0.2,-1.1,//3
		-0.7,0.3*Math.sin(time)-0.2,-1.5,//8
		-0.4,0.3*Math.sin(time)-0.2,-1.6,//10

		0,0.2*Math.sin(time)-0.2,-1.1,//3
		0.7,0.3*Math.sin(time)-0.2,-1.5,//9
		0.4,0.3*Math.sin(time)-0.2,-1.6,//11

		0,0.2*Math.sin(time)-0.2,-1.1,//3
		-0.4,0.3*Math.sin(time)-0.2,-1.6,//10
		-0.2,0.3*Math.sin(time)-0.2, -1.3,//12

		0,0.2*Math.sin(time)-0.2,-1.1,//3
		0.4,0.3*Math.sin(time)-0.2,-1.6,//11
		0.2,0.3*Math.sin(time)-0.2,-1.3//13
	];
	var vertices =[];

	for(var i = 0, i_max = 35; i <= i_max; i++)
	{
		vertices.push(vec3(verticesChange[3*i],verticesChange[3*i+1],verticesChange[3*i+2]));
	};	
	for(var i = 0, i_max = 35; i <= i_max; i++)
	{
		vertices.push(vec3(-verticesChange[3*i+2],verticesChange[3*i+1],verticesChange[3*i]));
	};
	for(var i = 0, i_max = 35; i <= i_max; i++)
	{
		vertices.push(vec3(verticesChange[3*i],verticesChange[3*i+1],-verticesChange[3*i+2]));
...
// function Leaf( points_transform)
// {
// 	shape.call(this);
// 	if(!arguments.length) return;
// 	this.populate(this,points_transform);
...
// Leaf.prototype.populate = function(recipient, points_transform)
// {
// 	var vertices =
// 	[
...
function cubeChange( points_transform )
{
	shape.call( this );
	if(!arguments.length) return;
	this.populate( this, points_transform );
...
inherit( cubeChange, shape);

cubeChange.prototype.populate = function( recipient, points_transform )
{
	var vertices =
	[
		0.5,0.5,0.5,//1
    ...
    var textures =
    [
    	vec2(1,1),vec2(0,1),vec2(1,0.8),//123
    	vec2(0,1),vec2(1,0.8),vec2(1,1),//234
    	vec2(1,1),vec2(0,1),vec2(1,0),//125
    	vec2(0,1),vec2(1,0),vec2(0,0),//256
    	vec2(1,1),vec2(0,1),vec2(1,0),//135
    	vec2(0,1),vec2(1,0),vec2(0,0),//357
    	vec2(0,1),vec2(1,1),vec2(1,0),//248
    	vec2(0,1),vec2(0,0),vec2(1,0),//268
    	vec2(0,1),vec2(1,1),vec2(1,0),//348
    	vec2(0,1),vec2(0,0),vec2(1,0),//378
    	vec2(0,1),vec2(0,0),vec2(0,0.5),//568
    	vec2(0,1),vec2(0,0),vec2(0,0.5)//578
    ];
    for ( var j = 0; j <= 35; j++)
    {
    	recipient.texture_coords.push(textures[j]);
    };
    var normal =
    [
    ...
    for ( var k = 0; k <= 35; k++)
    {
    	recipient.normals.push(normal[k]);
    };
